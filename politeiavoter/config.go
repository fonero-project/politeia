...
// Copyright (c) 2015-2019 The Fonero developers
...
import (
	...
	"github.com/fonero-project/politeia/util/version"
...
var (
	defaultHomeDir        = fnoutil.AppDataDir("politeiavoter", false)
	defaultConfigFile     = filepath.Join(defaultHomeDir, defaultConfigFilename)
	defaultLogDir         = filepath.Join(defaultHomeDir, defaultLogDirname)
	fnowalletHomeDir      = fnoutil.AppDataDir("fnowallet", false)
	defaultWalletCertFile = filepath.Join(fnowalletHomeDir, "rpc.cert")
)
...
// config defines the configuration options for fnod.
//
...
func loadConfig() (*config, []string, error) {
	...
	if cfg.PoliteiaWWW == "" {
		if activeNetParams.Name == "mainnet" {
			cfg.PoliteiaWWW = "https://proposals.fonero.org/api"
		} else {
			cfg.PoliteiaWWW = "https://test-proposals.fonero.org/api"
		}
	}
