...
type BEPrimaryTransactionVout struct {
	Amount       json.Number                      `json:"value"`        // Transaction amount (in FNO)
...
type BEBackupTransaction struct {
	...
	TxId          string      `json:"txid"`          // Transaction id
	Amount        json.Number `json:"amount"`        // Transaction amount (in FNO)
	Confirmations uint64      `json:"confirmations"` // Number of confirmations
	Timestamp     int64       `json:"ts"`            // Transaction timestamp
}

// TxDetails is an object representing a transaction that is used to
// standardize the different responses from fnodata and insight.
...
func FnoStringToAmount(fnostr string) (uint64, error) {
	...
	if !match {
		return 0, fmt.Errorf("invalid FNO amount: %v", fnostr)
	}

	var fnosplit []string
	if strings.Contains(fnostr, ".") {
		fnosplit = strings.Split(fnostr, ".")
		if len(fnosplit[0]) == 0 {
			fnosplit[0] = "0"
		}
	} else {
		fnosplit = []string{fnostr, "0"}
	}

	whole, err := strconv.ParseUint(fnosplit[0], 10, 64)
	if err != nil {
		return 0, err
	}

	fnosplit[1] += "00000000"
	fraction, err := strconv.ParseUint(fnosplit[1][0:8], 10, 64)
	if err != nil {
		return 0, err
	}
...
func fetchTxWithPrimaryBE(url string, address string, minimumAmount uint64, txnotbefore int64, minConfirmationsRequired uint64) (string, uint64, error) {
	responseBody, err := makeRequest(url, fnodataTimeout)
	if err != nil {
		return "", 0, err
	}
...
func PayWithTestnetFaucet(faucetURL string, address string, amount uint64, overridetoken string) (string, error) {
	fnoaddress, err := fnoutil.DecodeAddress(address)
	if err != nil {
	...
	fnoamount := strconv.FormatFloat(fnoutil.Amount(amount).ToCoin(),
		'f', -1, 32)

	// build request
	form := url.Values{}
	form.Add("address", address)
	form.Add("amount", fnoamount)
...
func FetchTxWithBlockExplorers(address string, amount uint64, txnotbefore int64, minConfirmations uint64) (string, uint64, error) {
	...
	fnodataURL, insightURL, err := blockExplorerURLForAddress(address,
		addr.Net())
	if err != nil {
		return "", 0, err
	}
	primaryURL := fnodataURL + "/raw"
	backupURL := insightURL + "/utxo?noCache=1"

	// Try the primary (fnodata) first.
	txID, amount, err := fetchTxWithPrimaryBE(primaryURL, address, amount,
		txnotbefore, minConfirmations)
	if err != nil {
		log.Printf("failed to fetch from fnodata: %v", err)
	} else {
...
// provided wallet address from the fnodata block explorer. If the fnodata
...
func FetchTxsForAddressNotBefore(address string, notBefore int64) ([]TxDetails, error) {
	...
	fnodataURL, _, err := blockExplorerURLForAddress(address, addr.Net())
	if err != nil {
		return nil, err
	}
	...
	for !done {
		...
		url := fnodataURL + "/count/" + strconv.Itoa(count) +
		...
		txs := make([]TxDetails, len(fnodataTxs))
		for _, tx := range fnodataTxs {
