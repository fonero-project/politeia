...
import (
	...
	"github.com/fonero-project/fnowallet/rpc/walletrpc"
...
func (cmd *TestRunCmd) Execute(args []string) error {
	...
	if paywallEnabled {
		...
		fno := float64(lr.PaywallAmount) / 1e8
		fmt.Printf("  Paid %v FNO to %v with txID %v\n",
			fno, lr.PaywallAddress, txID)
	}
	...
	if err != nil {
		...
		case strings.Contains(err.Error(), "connection refused"):
			// User is not running a fnowallet instance locally.
			// This is ok. Print a warning and continue.
			fmt.Printf("  WARNING: could not connect to fnowallet; " +
				"skipping cast votes\n")
			skipCastVotes = true
...
Paywall: 
If the politeiawww paywall is enabled the test run will use the Fonero tesnet
...
Voting:
The test run will attempt to vote on a proposal.  If a fnowallet instance is
