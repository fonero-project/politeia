...
`https://proposals.fonero.org`.  Copy these lines into your
...
`--paywall` will use the Fonero testnet faucet to satisfy the user registration
fee requirement. 
...
prevention measure that would normally need to be purchased using FNO, but if
...
$ politeiawww_dbutil -testnet -adfnoedits email@example.com 50
```
...
[politeiavoter](https://github.com/fonero-project/politeia/tree/master/politeiavoter/)
tool.

### Voting on a proposal - politeiawwwcli
You can also vote on proposals using `politeiawwwcli`, but for right now, it 
only works on testnet and you have to be running your fnowallet locally using
