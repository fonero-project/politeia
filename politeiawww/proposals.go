...
const (
	...
	// Note that 14 is in use by the fonero plugin
	// Note that 15 is in use by the fonero plugin
...
func voteStatusFromVoteSummary(r foneroplugin.VoteSummaryReply, bestBlock uint64) www.PropVoteStatusT {
	switch {
	case !r.Authorized:
		return www.PropVoteStatusNotAuthorized
	case r.EndHeight == "":
...
func (p *politeiawww) getProp(token string) (*www.ProposalRecord, error) {
	...
	dc, err := p.foneroGetComments(token)
	if err != nil {
		log.Errorf("getProp: foneroGetComments failed "+
			"for token %v", token)
	}
...
func (p *politeiawww) getPropVersion(token, version string) (*www.ProposalRecord, error) {
	...
	dc, err := p.foneroGetComments(token)
	if err != nil {
		log.Errorf("getPropVersion: foneroGetComments "+
...
func (p *politeiawww) getAllProps() ([]www.ProposalRecord, error) {
	...
	for _, v := range records {
		...
		dc, err := p.foneroGetComments(pr.CensorshipRecord.Token)
		if err != nil {
			log.Errorf("getAllProps: foneroGetComments failed "+
...
func (p *politeiawww) getPropComments(token string) ([]www.Comment, error) {
	...
	dc, err := p.foneroGetComments(token)
	if err != nil {
		return nil, fmt.Errorf("foneroGetComments: %v", err)
	}
...
func (p *politeiawww) processSetProposalStatus(sps www.SetProposalStatus, u *user.User) (*www.SetProposalStatusReply, error) {
	...
	case pr.State == www.PropStateVetted:
		...
		vdr, err := p.foneroVoteDetails(pr.CensorshipRecord.Token)
		if err != nil {
			return nil, fmt.Errorf("foneroVoteDetails: %v", err)
		}
		vd := convertVoteDetailsReplyFromFonero(*vdr)
...
func (p *politeiawww) processEditProposal(ep www.EditProposal, u *user.User) (*www.EditProposalReply, error) {
	...
	vdr, err := p.foneroVoteDetails(ep.Token)
	if err != nil {
		return nil, fmt.Errorf("foneroVoteDetails: %v", err)
	}
	vd := convertVoteDetailsReplyFromFonero(*vdr)
...
func (p *politeiawww) voteStatusReply(token string, bestBlock uint64) (*www.VoteStatusReply, error) {
	...
	r, err := p.foneroVoteSummary(token)
	if err != nil {
		return nil, err
	}

	results := convertVoteOptionResultsFromFonero(r.Results)
	var total uint64
...
func (p *politeiawww) processActiveVote() (*www.ActiveVoteReply, error) {
	...
	for _, v := range all {
		...
		vdr, err := p.foneroVoteDetails(v.CensorshipRecord.Token)
		if err != nil {
			log.Errorf("processActiveVote: foneroVoteDetails failed %v: %v",
...
func (p *politeiawww) processVoteResults(token string) (*www.VoteResultsReply, error) {
	...
	// Get vote details from cache
	vdr, err := p.foneroVoteDetails(token)
	if err != nil {
		return nil, fmt.Errorf("foneroVoteDetails: %v", err)
	}

	// Get cast votes from cache
	vrr, err := p.foneroProposalVotes(token)
	if err != nil {
		return nil, fmt.Errorf("foneroVoteDetails: %v", err)
	}
	...
	return &www.VoteResultsReply{
		...
		CastVotes:      convertCastVotesFromFonero(vrr.CastVotes),
	}, nil
}
...
func (p *politeiawww) processCastVotes(ballot *www.Ballot) (*www.BallotReply, error) {
	...
	payload, err := foneroplugin.EncodeBallot(convertBallotFromWWW(*ballot))
	if err != nil {
		return nil, err
	}
	pc := pd.PluginCommand{
		Challenge: hex.EncodeToString(challenge),
		ID:        foneroplugin.ID,
		Command:   foneroplugin.CmdBallot,
		CommandID: foneroplugin.CmdBallot,
	...
	// Decode plugin reply
	br, err := foneroplugin.DecodeBallotReply([]byte(reply.Payload))
	if err != nil {
		return nil, err
	}
	brr := convertBallotReplyFromFoneroPlugin(*br)
	return &brr, nil
}
...
// processAuthorizeVote sends the authorizevote command to fonero plugin to
...
func (p *politeiawww) processAuthorizeVote(av www.AuthorizeVote, u *user.User) (*www.AuthorizeVoteReply, error) {
	...
	// Get vote details from cache
	vdr, err := p.foneroVoteDetails(av.Token)
	if err != nil {
		return nil, fmt.Errorf("foneroVoteDetails: %v", err)
	}
	vd := convertVoteDetailsReplyFromFonero(*vdr)
	...
	payload, err := foneroplugin.EncodeAuthorizeVote(dav)
	if err != nil {
	...
	pc := pd.PluginCommand{
		Challenge: hex.EncodeToString(challenge),
		ID:        foneroplugin.ID,
		Command:   foneroplugin.CmdAuthorizeVote,
		CommandID: foneroplugin.CmdAuthorizeVote + " " + av.Token,
		Payload:   string(payload),
	}
	...
	// Decode plugin reply
	avr, err := foneroplugin.DecodeAuthorizeVoteReply([]byte(reply.Payload))
	if err != nil {
...
func (p *politeiawww) processStartVote(sv www.StartVote, u *user.User) (*www.StartVoteReply, error) {
	...
	payload, err := foneroplugin.EncodeStartVote(dsv)
	if err != nil {
		return nil, err
	}
	...
	// Get vote details from cache
	vdr, err := p.foneroVoteDetails(sv.Vote.Token)
	if err != nil {
		return nil, fmt.Errorf("foneroVoteDetails: %v", err)
	}
	vd := convertVoteDetailsReplyFromFonero(*vdr)
	...
	// Tell fonero plugin to start voting
	challenge, err := util.Random(pd.ChallengeSize)
	if err != nil {
		return nil, err
	}
	...
	pc := pd.PluginCommand{
		Challenge: hex.EncodeToString(challenge),
		ID:        foneroplugin.ID,
		Command:   foneroplugin.CmdStartVote,
		CommandID: foneroplugin.CmdStartVote + " " + sv.Vote.Token,
		Payload:   string(payload),
	}
	...
	vr, err := foneroplugin.DecodeStartVoteReply([]byte(reply.Payload))
	if err != nil {
		return nil, err
	}
	...
	// return a copy
	rv := convertStartVoteReplyFromFonero(*vr)
	return &rv, nil
}
...
func (p *politeiawww) processTokenInventory() (*www.TokenInventoryReply, error) {
	...
	for retries := 0; !done && retries <= 1; retries++ {
		...
		r = convertTokenInventoryReplyFromFonero(*ti)
		done = true
	}
