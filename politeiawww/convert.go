...
import (
	...
	"github.com/fonero-project/politeia/politeiawww/cmsdatabase"
)

func convertCastVoteReplyFromFoneroPlugin(cvr foneroplugin.CastVoteReply) www.CastVoteReply {
...
func convertCastVoteFromWWW(b www.CastVote) foneroplugin.CastVote {
...
func convertBallotFromWWW(b www.Ballot) foneroplugin.Ballot {
	br := foneroplugin.Ballot{
		Votes: make([]foneroplugin.CastVote, 0, len(b.Votes)),
	}
...
func convertBallotReplyFromFoneroPlugin(b foneroplugin.BallotReply) www.BallotReply {
	br := www.BallotReply{
		Receipts: make([]www.CastVoteReply, 0, len(b.Receipts)),
	}
	for _, v := range b.Receipts {
		br.Receipts = append(br.Receipts,
			convertCastVoteReplyFromFoneroPlugin(v))
	}
	return br
}

func convertVoteOptionFromWWW(vo www.VoteOption) foneroplugin.VoteOption {
	return foneroplugin.VoteOption{
		Id:          vo.Id,
...
func convertVoteOptionsFromWWW(vo []www.VoteOption) []foneroplugin.VoteOption {
	vor := make([]foneroplugin.VoteOption, 0, len(vo))
	for _, v := range vo {
...
func convertVoteFromWWW(v www.Vote) foneroplugin.Vote {
	return foneroplugin.Vote{
		Token:            v.Token,
		Mask:             v.Mask,
		Duration:         v.Duration,
...
func convertAuthorizeVoteFromWWW(av www.AuthorizeVote) foneroplugin.AuthorizeVote {
...
func convertStartVoteFromWWW(sv www.StartVote) foneroplugin.StartVote {
	return foneroplugin.StartVote{
...
func convertNewCommentToFoneroPlugin(nc www.NewComment) foneroplugin.NewComment {
	return foneroplugin.NewComment{
		Token:     nc.Token,
...
func convertLikeCommentToFonero(lc www.LikeComment) foneroplugin.LikeComment {
...
func convertLikeCommentFromFonero(lc foneroplugin.LikeComment) www.LikeComment {
...
func convertCensorCommentToFonero(cc www.CensorComment) foneroplugin.CensorComment {
	return foneroplugin.CensorComment{
		Token:     cc.Token,
...
func convertCommentFromFonero(c foneroplugin.Comment) www.Comment {
...
func convertAuthVoteFromFonero(dav foneroplugin.AuthorizeVote) (www.AuthorizeVote, www.AuthorizeVoteReply) {
...
func convertStartVoteFromFonero(sv foneroplugin.StartVote) www.StartVote {
...
func convertStartVoteReplyFromFonero(svr foneroplugin.StartVoteReply) www.StartVoteReply {
...
func convertVoteDetailsReplyFromFonero(vdr foneroplugin.VoteDetailsReply) VoteDetails {
	av, avr := convertAuthVoteFromFonero(vdr.AuthorizeVote)
	return VoteDetails{
		AuthorizeVote:      av,
		AuthorizeVoteReply: avr,
		StartVote:          convertStartVoteFromFonero(vdr.StartVote),
		StartVoteReply:     convertStartVoteReplyFromFonero(vdr.StartVoteReply),
	}
}

func convertCastVoteFromFonero(cv foneroplugin.CastVote) www.CastVote {
	return www.CastVote{
		Token:     cv.Token,
		Ticket:    cv.Ticket,
		VoteBit:   cv.VoteBit,
...
func convertCastVotesFromFonero(cv []foneroplugin.CastVote) []www.CastVote {
	cvr := make([]www.CastVote, 0, len(cv))
	for _, v := range cv {
		cvr = append(cvr, convertCastVoteFromFonero(v))
	}
	return cvr
}

func convertVoteResultsReplyFromFonero(vrr foneroplugin.VoteResultsReply) (www.StartVote, []www.CastVote) {
	sv := convertStartVoteFromFonero(vrr.StartVote)
	cv := convertCastVotesFromFonero(vrr.CastVotes)
	return sv, cv
}
...
func convertVoteOptionResultsFromFonero(vor []foneroplugin.VoteOptionResult) []www.VoteOptionResult {
...
func convertTokenInventoryReplyFromFonero(r foneroplugin.TokenInventoryReply) www.TokenInventoryReply {
