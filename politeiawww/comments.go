...
func (p *politeiawww) initCommentScores() error {
	...
	// Fetch fonero plugin inventory from cache
	ir, err := p.foneroInventory()
	if err != nil {
		return fmt.Errorf("foneroInventory: %v", err)
	}
...
func (p *politeiawww) getComment(token, commentID string) (*www.Comment, error) {
	...
	dc, err := p.foneroGetComment(token, commentID)
	if err != nil {
		return nil, fmt.Errorf("foneroGetComment: %v", err)
	}
	c := convertCommentFromFonero(*dc)
...
func (p *politeiawww) updateCommentScore(token, commentID string) (int64, error) {
	...
	likes, err := p.foneroCommentLikes(token, commentID)
	if err != nil {
		return 0, fmt.Errorf("foneroLikeComments: %v", err)
	}
...
// processNewComment sends a new comment fonero plugin command to politeaid
...
func (p *politeiawww) processNewComment(nc www.NewComment, u *user.User) (*www.NewCommentReply, error) {
	...
	vdr, err := p.foneroVoteDetails(nc.Token)
	if err != nil {
		return nil, fmt.Errorf("foneroVoteDetails: %v", err)
	}
	_, avr := convertAuthVoteFromFonero(vdr.AuthorizeVote)
	svr := convertStartVoteReplyFromFonero(vdr.StartVoteReply)
	...
	dnc := convertNewCommentToFoneroPlugin(nc)
	payload, err := foneroplugin.EncodeNewComment(dnc)
	if err != nil {
		return nil, err
	}
	...
	pc := pd.PluginCommand{
		...
		ID:        foneroplugin.ID,
		Command:   foneroplugin.CmdNewComment,
		CommandID: foneroplugin.CmdNewComment,
	...
	ncr, err := foneroplugin.DecodeNewCommentReply([]byte(reply.Payload))
	if err != nil {
		return nil, err
	}
...
// processNewCommentInvoice sends a new comment fonero plugin command to politeaid
// then fetches the new comment from the cache and returns it.
...
func (p *politeiawww) processLikeComment(lc www.LikeComment, u *user.User) (*www.LikeCommentReply, error) {
	...
	// Ensure proposal voting has not ended
	vdr, err := p.foneroVoteDetails(lc.Token)
	if err != nil {
		return nil, fmt.Errorf("foneroVoteDetails: %v", err)
	}
	...
	_, err = p.foneroGetComment(lc.Token, lc.CommentID)
	if err != nil {
		if err == cache.ErrRecordNotFound {
			err = www.UserError{
	...
	dlc := convertLikeCommentToFonero(lc)
	payload, err := foneroplugin.EncodeLikeComment(dlc)
	if err != nil {
		return nil, err
	}
	...
	pc := pd.PluginCommand{
		Challenge: hex.EncodeToString(challenge),
		ID:        foneroplugin.ID,
		Command:   foneroplugin.CmdLikeComment,
		CommandID: foneroplugin.CmdLikeComment,
		Payload:   string(payload),
	}
	...
	lcr, err := foneroplugin.DecodeLikeCommentReply([]byte(reply.Payload))
	if err != nil {
		return nil, err
	}
...
// processCensorComment sends a censor comment fonero plugin command to
...
func (p *politeiawww) processCensorComment(cc www.CensorComment, u *user.User) (*www.CensorCommentReply, error) {
	...
	c, err := p.foneroGetComment(cc.Token, cc.CommentID)
	if err != nil {
		return nil, fmt.Errorf("foneroGetComment: %v", err)
	}
	if c.Censored {
	...
	// Ensure proposal voting has not ended
	vdr, err := p.foneroVoteDetails(cc.Token)
	if err != nil {
		return nil, fmt.Errorf("foneroVoteDetails: %v", err)
	}
	vd := convertVoteDetailsReplyFromFonero(*vdr)
	...
	dcc := convertCensorCommentToFonero(cc)
	payload, err := foneroplugin.EncodeCensorComment(dcc)
	if err != nil {
		return nil, err
	}
	...
	pc := pd.PluginCommand{
		Challenge: hex.EncodeToString(challenge),
		ID:        foneroplugin.ID,
		Command:   foneroplugin.CmdCensorComment,
		CommandID: foneroplugin.CmdCensorComment,
		Payload:   string(payload),
	}
	...
	ccr, err := foneroplugin.DecodeCensorCommentReply([]byte(reply.Payload))
	if err != nil {
		return nil, err
	}
