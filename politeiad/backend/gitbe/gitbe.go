// Copyright (c) 2017-2018 The Fonero developers
...
import (
	...
	"github.com/fonero-project/fnotime/api/v1"
	...
	"github.com/fonero-project/politeia/politeiad/api/v1/mime"
...
// anchor takes a slice of commit digests and anchors them in fnotime.
//
// This function is being clever with the anchors.  It sends two values to
// fnotime.  We anchor the merkle root, and we *also* anchor all
...
// truly curious.  This is essentially free because fnotime compresses all
// digests into a single merkle root.
//
// This function should be called with the lock held.
// TODO: the physical write to fnotime needs to come out of the lock.
func (g *gitBackEnd) anchor(digests []*[sha256.Size]byte) error {
	...
	return util.Timestamp(g.fnotimeHost, digests)
}
...
// case it drops and anchor in fnotime for each of them.
...
func (g *gitBackEnd) afterAnchorVerify(vrs []v1.VerifyDigest) error {
	...
	for _, vr := range vrs {
		if vr.ChainInformation.ChainTimestamp == 0 {
			// fnotime returns 0 when there are not enough
			// confirmations yet.
		...
		// Mark test anchors as confirmed by fnotime
		if g.test {
...
// verifyAnchor asks fnotime if an anchor has been verified and returns a TX if
// it has.
func (g *gitBackEnd) verifyAnchor(digest string) (*v1.VerifyDigest, error) {
	...
	if g.test {
		...
		// Call fnotime
		vr, err = util.Verify(g.fnotimeHost, []string{digest})
		if err != nil {
			return nil, err
		}
	}
...
func (g *gitBackEnd) _updateRecord(commit bool, id string, mdAppend, mdOverwrite []backend.MetadataStream, fa []file, filesDel []string) error {
	...
	avFilename := fmt.Sprintf("%02v%v", foneroplugin.MDStreamAuthorizeVote,
		defaultMDFilenameSuffix)
	...
	// Call plugin hooks
	f, ok := foneroPluginHooks[PluginPostHookEdit]
	if ok {
...
// fnotime.  This is an expensive operation and should not be run during
// runtime.
//
...
func (g *gitBackEnd) fsck(path string) error {
	...
	var seenAnchor bool
	// gitDigests is an index of all git digests to verify with fnotime
	gitDigests := make(map[string]struct{})
	// confirmedAnchors keeps track of anchors that were timestamped with fnotime but not verified,
	...
	for _, v := range out {
		if regexAnchorConfirmation.MatchString(v) {
			...
			// If we didn't, save the anchor key to verify with fnotime later
	...
	log.Infof("fsck: fnotime verification started")
	...
	vr, err := util.Verify(g.fnotimeHost, digests)
	if err != nil {
		return err
	}
	...
	for _, v := range vr.Digests {
		if v.Result != v1.ResultOK {
			fail = true
			log.Errorf("fnotime error: %v %v %v", v.Digest,
				v.Result, v1.Result[v.Result])
		}
	}
	if fail {
		return fmt.Errorf("fnotime fsck failed")
	}
...
func New(anp *chaincfg.Params, root string, fnotimeHost string, gitPath string, id *identity.FullIdentity, gitTrace bool) (*gitBackEnd, error) {
	...
	g := &gitBackEnd{
		...
		gitPath:         gitPath,
		fnotimeHost:     fnotimeHost,
		gitTrace:        gitTrace,
		exit:            make(chan struct{}),
		checkAnchor:     make(chan struct{}),
		testAnchors:     make(map[string]bool),
		plugins:         []backend.Plugin{getFoneroPlugin(anp.Name != "mainnet")},
	}
	idJSON, err := id.Marshal()
	if err != nil {
		return nil, err
	}
	setFoneroPluginSetting(foneroPluginIdentity, string(idJSON))
	setFoneroPluginSetting(foneroPluginJournals, g.journals)
	setFoneroPluginHook(PluginPostHookEdit, g.foneroPluginPostEdit)
	...
	err = g.initFoneroPluginJournals()
	if err != nil {
		return nil, err
	}
	...
	// Message user
	log.Infof("Timestamp host: %v", g.fnotimeHost)

	log.Infof("Running fnotime fsck on vetted repository")
	err = g.fsck(g.vetted)
	if err != nil {
		// Log error but continue
		log.Errorf("fsck: fnotime %v", err)
	}
